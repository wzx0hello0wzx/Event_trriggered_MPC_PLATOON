function [sys,x0,str,ts] = Car3_MPC_Longitudinal_Control(t,x,u,flag)
...
function sys=mdlOutputs(t,x,u)
 ...
 global a b u_piao3;  %a、b分别表示线性化后的状态矩阵和输入矩阵 u_piao是计算出来的值
 global U3 k3;   %控制输入 速度和转角
 global kesi3;
 tic
%     profile on;
    Nx=3;   %状态量的个数
    ...
    %矩阵的处理，先分配空间，再向空间幅值
    kesi3=zeros(Nx+Nu,1);    %组合后的状态量
    kesi3(1)=u(1)-r(1);  %u(1)==X(1)=x 当前时刻的状态
    kesi3(2)=u(2)-r(2);  %u(2)==X(2)=y
    kesi3(3)=u(3)-r(3);   %u(3)==X(3)=phi
    kesi3(4)=U3(1);   %上一时刻的控制输入v
    fprintf('kesi3=%4.2f\n',kesi3);
    %kesi(5)=U(2);   %上一时刻的控制输入delta_f
    fprintf('Update start3, u(1)=%4.2f\n',U3(1))
    ...
    %矩阵初始化
    % ==========================================================
    u_piao3=zeros(Nx,Nu);  %u_piao是计算出来的值
     persistent u_real3 V3 Event_t3 Event_k3 eve3;
 if(isempty(u_real3))
     u_real3=zeros(Nc,Nu);
     V3=zeros(Nc,Nu);
     Event_k3=[];
...
%     Q=100*eye(Nx*Np,Nx*Np); 
    ...
    for j=1:1:Np	
        PHI_cell{j,1}=C*A^j;
        for z3=1:1:Nc	%在给THETA赋值的时候，书上的THATA矩阵表述与这里不同。
            if z3<=j
                THETA_cell{j,z3}=C*A^(j-z3)*B;
            else 
                THETA_cell{j,z3}=zeros(Nx,Nu);
            end
        end
    end
...
%     f=(cell2mat(f_cell))';
 ...
 %不等式约束
    ...
    Ut=kron(ones(Nc,1),U3);%此处感觉论文里的克罗内科积有问题,暂时交换顺序
    umin=[-2];%维数与控制变量的个数相同
    umax=[2];
    delta_umin=[-0.2];
    delta_umax=[0.2];
...
%     options = optimset('Algorithm','active-set');
    ...
    %% 计算输出，反馈
    for l=1:1:Nc
    u_piao3(l)=X(l); %计算出的控制增量v
    %u_piao(2)=X(2); %计算出的控制增量delta
    U3(1)=kesi3(4)+u_piao3(1);%用于存储上一个时刻的控制量v
    u_real3(l)=kesi3(4)+u_piao3(l)+vd1;%u(k)
    end
    k3=k3+1;
    fprintf('k3=%4.2f\n',k3);
    sys = u_real3(1);
    ...
    for d=1:1:Nc
        V3(d)=Y(2+(d-1)*3);%获取速度偏差值序列
    end

  elseif (k3>1 && k3<=30 && (abs(0-V3(k3))<=w))
        sys = u_real3(k3);
        k3=k3+1;
        fprintf('k3=%4.2f\n',k3);
    end
...
savePath = strcat(fileDir, num2str(3), '_result.mat'); % 拼接路径和文件名
% num2str(n) 在循环中可以生成1_result.mat, 2_result.mat....
% 拼接后的文件为 ： 文件路径/1_result.mat
save(savePath, 'Event_t3', 'Event_k3'); % 保存变量val1,val2到1_result.mat中
% end mdlOutputs